...
- `notifier` object can be used as the only representation of `"TheParameter"` in the whole program
- `notifier.value` allows indirectly read and write the shared preference value without getting out of sync
- `Widget build(_)` methods can access value without relying on `FutureBuilder`
- `notifier.addListener` makes it possible to track changes of the value
...
``` dart
final notifier = PrefItem<int>(SharedPrefsStorage(), "TheParameter");
```

### Read PrefNotifier value

Reading is is not finished yet. But we already can access `notifier.value`. By default, it returns `null`.
...
Widget build(BuildContext context) {
    if (notifier.value==null)
        ...
        return Text("Value is ${notifier.value}");
}
```

Since `PrefNotifier` inherits from the `ValueNotifier`, we can automatically 
rebuild the widget when the new value of `notifier` will be available:
...
// Now notifier.value returns the value from the storage, not default NULL.
